const CONFIG = {
  ...
  concertDate : "2026-07-23T09:00:00+09:00",
  concertEndDate : "2026-07-23T10:00:00+09:00",   // 공연 종료(이후 마무리 문구)
...
const LS_COLORS=[
  ...
  {k:"white", name:"화이트", rgb:[255,255,255]},
  {k:"blue",  name:"블루",   rgb:[120,170,255]},
  {k:"violet",name:"바이올렛",rgb:[185,150,255]},
...
function renderLsColors(order){
  ...
  if(Array.isArray(order)&&order.length){
    const byKey=order.map(k=>LS_COLORS.find(c=>c.k===k)).filter(Boolean);
    LS_COLORS.forEach(c=>{if(!byKey.includes(c))byKey.push(c);});   // 누락 색 뒤에 보강
    list=byKey;
  }
  list.forEach(c=>{
...
function lsSyncSel(){
  ...
  [...lsColorsBox.children].forEach(el=>{const c=LS_COLORS.find(x=>x.k===el.dataset.k);el.classList.toggle("sel", c&&c.rgb.join(",")===cur);});
}
renderLsColors();
...
(async function lsLiveSubscribe(){
  ...
  onSnapshot(doc(db,"live","state"), snap=>{
    ...
    if(Array.isArray(d.order)) renderLsColors(d.order);
